...
def ej2():
    ...
    for epoch in range(epochs):
        ...
        total_count = 0
        for line in reader:
            data = np.array([float(x)/255 for x in line[1:]])
    ...
    for i, elem in enumerate(test_reader):
        ...
        data = [float(x)/255 for x in elem[1:]]
...
def graph_latent_space(ae):
    ...
    grid_x = np.linspace(-0.25, 1.25, n)
    grid_y = np.linspace(-0.25, 1.25, n)[::-1]
